...
namespace HomaGames.HomaBelly
{
    ...
    public static class DefaultAnalytics
    {
        ...
        public static void SuggestedRewardedAd(string placementName)
        {
            HomaBelly.Instance.TrackDesignEvent("Rewarded:Suggested:" + Sanitize(placementName) + ":" + _sanitizedCurrentLevelId);
        }
        ...
        public static void RewardedAdTriggered(string placementName)
        {
            _sanitizedCurrentRewardedAdName = Sanitize(placementName);
            HomaBelly.Instance.TrackDesignEvent("Rewarded:Triggered:" + _sanitizedCurrentRewardedAdName + ":" + _sanitizedCurrentLevelId);
        }
        ...
        /// </summary>
        /// <param name="placementName"></param>
        public static void InterstitialAdTriggered(string placementName)
        {
            _sanitizedCurrentInterstitialAdName = Sanitize(placementName);
            HomaBelly.Instance.TrackDesignEvent("Interstitial:Triggered:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
        }
        ...
        private static void RegisterAdEvents()
        {
            ...
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:Ready:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onInterstitialAdLoadFailedEvent += () =>
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:LoadFailed:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onInterstitialAdClosedEvent += () =>
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:Closed:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onInterstitialAdClickedEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:Clicked:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onInterstitialAdShowFailedEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:ShowFailed:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onInterstitialAdShowSucceededEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Interstitials:ShowSucceeded:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId);

                if (PlayerPrefs.GetInt(INTERSTITIAL_AD_FIRST_TIME_KEY, 0) == 0)
                {
                    HomaBelly.Instance.TrackDesignEvent("Interstitials:FirstWatched:" + _sanitizedCurrentInterstitialAdName + ":" + _sanitizedCurrentLevelId, GetTotalGameplaySecondsAtThisExactMoment());
            ...
            Events.onRewardedVideoAdRewardedEvent += (reward) =>
            {
                if (!string.IsNullOrEmpty(_sanitizedCurrentRewardedAdName))
                {
                    ...
                    if (!rewardedAdsTakenThisSession.ContainsKey(_sanitizedCurrentRewardedAdName))
                    {
                        ...
                        HomaBelly.Instance.TrackDesignEvent("Rewarded:FirstWatchedSession:" + _sanitizedCurrentRewardedAdName, GetTotalGameplaySecondsAtThisExactMoment());
                    }

                    // Current rewarded ad has not been taken ever
                    if (PlayerPrefs.GetInt(REWARDED_AD_FIRST_TIME_TAKEN_EVER_KEY, 0) == 0)
                    {
                        HomaBelly.Instance.TrackDesignEvent("Rewarded:FirstWatched:" + _sanitizedCurrentRewardedAdName, GetTotalGameplaySecondsAtThisExactMoment());
                        PlayerPrefs.SetInt(REWARDED_AD_FIRST_TIME_TAKEN_EVER_KEY, 1);
                        PlayerPrefs.Save();
                    }
                }
                
                HomaBelly.Instance.TrackDesignEvent("Rewarded:Taken:" + _sanitizedCurrentRewardedAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onRewardedVideoAdShowFailedEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Rewarded:Failed:" + _sanitizedCurrentRewardedAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onRewardedVideoAdStartedEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Rewarded:Opened:" + _sanitizedCurrentRewardedAdName + ":" + _sanitizedCurrentLevelId);
            };

            Events.onRewardedVideoAdClosedEvent += (id) =>
            {
                HomaBelly.Instance.TrackDesignEvent("Rewarded:Closed:" + _sanitizedCurrentRewardedAdName + ":" + _sanitizedCurrentLevelId);
            };
        }
