...
using UnityEditor;
using UnityEngine;
using EditorAnalytics = HomaGames.HomaBelly.EditorAnalytics;
        ...
        else
        {
            if (GUILayout.Button(buttonContent, HomaGamesStyle.ButtonStyleTexts))
            {
                if (IsValidAppToken(appToken))
                {
                    ...
                    ApiQueryModel.TokenIdentifier = appToken;
                    EditorAnalytics.TrackEditorAnalyticsEvent("manifest_fetching_started");
                    homaBellyWindowController.RequestPluginManifest(appToken).ContinueWith((taskResult) =>
                    {
                        try
                        {
                            ...
                            {
                                EditorAnalytics.TrackEditorAnalyticsEvent("manifest_fetching_completed");
                            ...
                            {
                                EditorAnalytics.TrackEditorAnalyticsEvent("manifest_fetching_error", "Error deserializing. Manifest is null");
                        ...
                        catch (AggregateException ae)
                        {
                            ...
                            {
                                EditorAnalytics.TrackEditorAnalyticsEvent("manifest_fetching_error", e.Message);
