...
namespace HomaGames.HomaBelly
{
    ...
    public class HomaBellyUpdateChecker
    {
            ...
            if (oldPluginManifest != null)
            {
                FetchPluginManifest(oldPluginManifest.AppToken).ContinueWith((manifestResult) =>
                {
                    ...
                    if (newPluginManifest != null)
                    {
                        ...
                        if (_updateAvailable)
                        {
                            EditorAnalytics.TrackEditorAnalyticsEvent("manifest_update_available");
        ...
        private static Task<PluginManifest> FetchPluginManifest(string token)
        {
            try
            {
                return new EditorHttpCaller<PluginManifest>().Get(string.Format(HomaBellyConstants.API_APP_BASE_URL, token), new PluginManifestDeserializer(false));
            }
        ...
        private static void TryShowUpdatePopup()
        {
            if (ShouldShowUpdatePopup())
            {
                ...
                {
                    EditorAnalytics.TrackEditorAnalyticsEvent("manifest_update_available_popup_prompted");
                    ...
                    if (updateHomaBelly)
                    {
                        EditorAnalytics.TrackEditorAnalyticsEvent("manifest_update_available_popup_accept");
                    ...
                    {
                        EditorAnalytics.TrackEditorAnalyticsEvent("manifest_update_available_popup_deny");
