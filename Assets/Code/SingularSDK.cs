...
public class SingularSDK : MonoBehaviour {
    ...
    private const string UNITY_VERSION = "3.0.0";
	...
	*/
    public static async void Event(Dictionary<string, object> args, string name) {
        ...
        if (!Application.isEditor) {
            ...
            await System.Threading.Tasks.Task.Run(delegate
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    else
                    {
                        ...
                        {
                            type = NSType.DICTIONARY;
                            CreateDictionary(-1, NSType.DICTIONARY, enumerator.Current.Key,
                                (Dictionary<string, object>) enumerator.Current.Value);
                        }
                        else if (valueType == typeof(ArrayList))
                        {
                            type = NSType.ARRAY;
                            CreateArray(-1, NSType.DICTIONARY, enumerator.Current.Key,
                                (ArrayList) enumerator.Current.Value);
                        }
            ...
            await Task.Run(() => jsonString = JsonConvert.SerializeObject(args, Formatting.None));
            
            AndroidJavaObject json = new AndroidJavaObject("org.json.JSONObject", jsonString);
